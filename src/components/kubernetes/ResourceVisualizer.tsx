...
export function ResourceVisualizer({
  ...
  const sections = useMemo(() => adaptResource(context, resource), [context, resource]);
  const actions = useMemo(() => getResourceActions(resource), [resource]);
  ...
  if (!sections || sections.sections.length === 0) {
    return (
      <div className="text-neutral-500 text-sm">No visualization available for {resource.kind}</div>
    );
  }
