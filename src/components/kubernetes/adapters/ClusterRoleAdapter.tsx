...
function RulesTable({ rules, title }: { rules: V1PolicyRule[]; title?: string }) {
  if (!rules || rules.length === 0) {
    return (
      <div className="text-xs text-neutral-500 dark:text-neutral-500 italic">No rules defined</div>
    );
  }
...
function AggregatedRolesSection({
  ...
  if (loading) {
    return <div className="text-xs text-neutral-500">Loading aggregated roles...</div>;
  }
...
export const ClusterRoleAdapter: ResourceAdapter<V1ClusterRole> = {
  ...
  adapt(context: string, resource: V1ClusterRole): ResourceSections {
    ...
    const isAggregated =
      ...
      aggregationRule.clusterRoleSelectors.length > 0;
