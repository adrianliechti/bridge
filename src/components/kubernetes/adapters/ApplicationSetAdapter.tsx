...
function getOverallStatus(conditions?: ApplicationSetStatus['conditions']): {
  status: string;
  level: StatusLevel;
} {
...
function getGeneratorInfo(generator: ApplicationSetGenerator): {
  name: string;
  ...
  if (generator.clusters) {
    return {
      ...
      details: generator.clusters.selector?.matchLabels
        ? Object.entries(generator.clusters.selector.matchLabels)
            .map(([k, v]) => `${k}=${v}`)
            .join(', ')
        : 'All clusters',
    };
  ...
  if (generator.scmProvider) {
    const provider = generator.scmProvider.github
      ? 'GitHub'
      : generator.scmProvider.gitlab
        ? 'GitLab'
        : generator.scmProvider.bitbucket
          ? 'Bitbucket'
          : generator.scmProvider.bitbucketServer
            ? 'Bitbucket Server'
            : generator.scmProvider.gitea
              ? 'Gitea'
              : generator.scmProvider.azureDevOps
                ? 'Azure DevOps'
                : 'Unknown';
    return {
      ...
      details:
        generator.scmProvider.github?.organization ||
  ...
  if (generator.pullRequest) {
    const provider = generator.pullRequest.github
      ? 'GitHub'
      : generator.pullRequest.gitlab
        ? 'GitLab'
        : generator.pullRequest.bitbucketServer
          ? 'Bitbucket Server'
          : 'Unknown';
    return {
      ...
      details:
        generator.pullRequest.github?.repo ||
            ...
            items: [
              ...
              },
              {
                ...
                icon: (
                  <Package
                    size={14}
                    className={applicationCount > 0 ? 'text-emerald-400' : 'text-gray-400'}
                  />
                ),
              },
              {
                    ...
                    return (
                      <div className="space-y-1 max-h-60 overflow-y-auto">
                        {status!.applicationStatus!.map((appStatus, idx) => {
                          const statusLevel =
                            appStatus.status === 'Successful' || appStatus.status === 'Healthy'
                              ? 'success'
          ...
          ? [
              {
                ...
                data: {
                  ...
                  items: [
                    {
                      label: 'Repository',
                      value: getRepoName(source.repoURL),
                      color: 'text-cyan-400',
                    },
                    {
                      label: 'Path',
                  ...
                  items: [
                    {
                      label: 'Server',
                      value:
                        template.spec.destination.name ||
                        template.spec.destination.server ||
                        '{{server}}',
                    },
                    {
                      label: 'Namespace',
                      value: template.spec.destination.namespace || '{{namespace}}',
                      color: 'text-cyan-400',
                    },
                    {
                      label: 'Project',
                      value: template.spec.project || 'default',
                      color: 'text-purple-400',
                    },
                  ],
                  columns: 2 as const,
                  ...
                  items: [
                    {
                      ...
                      color: spec.syncPolicy.preserveResourcesOnDeletion
                        ? 'text-amber-400'
                        : undefined,
                    },
