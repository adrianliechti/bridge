...
export function VolumeCard({ volume }: { volume: VolumeData }) {
  ...
  return (
    <div
      className={`border rounded-lg overflow-hidden ${typeStyles[volume.type] || 'border-neutral-300 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-900/50'}`}
    >
      <button
      ...
      >
        {typeIcons[volume.type] || (
          <HardDrive size={14} className="text-neutral-600 dark:text-neutral-400" />
        )}
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-neutral-600 dark:text-neutral-500 truncate">
            {volume.source && (
              <span className="text-cyan-600 dark:text-cyan-400/70">{volume.source}</span>
            )}
        ...
        <div className="border-t border-neutral-200 dark:border-neutral-800 p-3 space-y-3">
          ...
          <div>
            ...
            <div className="text-xs">
              <span
                className={`px-1.5 py-0.5 rounded ${typeBadgeStyles[volume.type] || 'bg-neutral-200 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-400'}`}
              >
                {volume.type}
              </span>
              {volume.source && (
              ...
              <div className="mt-2 text-xs space-y-1">
                {Object.entries(volume.extra).map(([key, value]) => (
                  <div key={key} className="text-neutral-600 dark:text-neutral-400">
                    ...
                    <span className="text-neutral-900 dark:text-neutral-300">{String(value)}</span>
                  </div>
                ))}
              </div>
            )}
                  ...
                  >
                    ...
                    {mount.subPath && (
                      <div className="ml-4 mt-1 text-neutral-600 dark:text-neutral-500">
                        <span className="text-neutral-600 dark:text-neutral-500">subPath:</span>{' '}
