  ...
  if (loading) {
    return (
      <div>
        ...
        <div className="text-xs text-neutral-500">Loading ReplicaSets...</div>
      </div>
    );
  }
...
export function RelatedPVCsSection({ loader, title }: { loader: () => Promise<PVCData[]>; title?: string }) {
  ...
  if (loading) {
    return (
      <div>
        {title && <h5 className="text-xs font-medium text-neutral-500 uppercase tracking-wider mb-2">{title}</h5>}
        <div className="text-xs text-neutral-500">Loading PVCs...</div>
      </div>
    );
  }
...
export function RelatedJobsSection({ loader, title }: { loader: () => Promise<JobData[]>; title?: string }) {
  ...
  if (loading) {
    return (
      <div>
        {title && <h5 className="text-xs font-medium text-neutral-500 uppercase tracking-wider mb-2">{title}</h5>}
        <div className="text-xs text-neutral-500">Loading jobs...</div>
      </div>
    );
  }
