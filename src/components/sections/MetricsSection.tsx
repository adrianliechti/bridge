...
import { HardDrive, Cpu, RefreshCw } from 'lucide-react';
...
export function NodeMetricsSection({
  ...
  loader: () => Promise<MetricsData | null>;
  title?: string;
}) {
  ...
  const cpuPercentage = calculatePercentage(
    data.cpu.usageNanoCores,
    data.cpu.allocatableNanoCores ?? 0,
  );
  const memoryPercentage = calculatePercentage(
    data.memory.usageBytes,
    data.memory.allocatableBytes ?? 0,
  );
      ...
      <div className="space-y-3">
        ...
        <div className="bg-neutral-900/50 rounded-lg p-3 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono">
              <span className={cpuPercentage > 80 ? 'text-amber-400' : 'text-blue-400'}>
                {data.cpu.usage}
              </span>
        ...
        <div className="bg-neutral-900/50 rounded-lg p-3 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono">
              <span className={memoryPercentage > 80 ? 'text-amber-400' : 'text-purple-400'}>
                {data.memory.usage}
              </span>
