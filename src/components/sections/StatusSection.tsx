...
import type { StatusCardData, GaugeData, GridData } from './types';
    ...
    <div className="grid grid-cols-2 gap-3">
      {items.map((item, i) => (
        <div key={i} className="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-3">
          ...
          <div
            className={`text-sm font-medium flex items-center gap-2 ${statusColors[item.status || 'neutral']}`}
          >
            {item.icon}
            {item.value}
          </div>
          ...
          return (
            <div key={i} className="flex-1">
              ...
              <div className="h-1 bg-neutral-300 dark:bg-neutral-700 rounded-full overflow-hidden">
                <div
                  className={`h-full ${bgClasses[item.color]}`}
