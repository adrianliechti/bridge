...
import { ChevronDown, ChevronRight, Network, GitBranch, Link } from 'lucide-react';
      ...
      {expanded && (
        <div className="border-t border-neutral-200 dark:border-neutral-800 p-3 space-y-3">
          {rule.matches && rule.matches.length > 0 && (
            <div>
              <div className="text-xs text-neutral-500 mb-1">gRPC Method Matches</div>
              <div className="space-y-2">
                  ...
                  <div key={i} className="text-xs bg-neutral-100 dark:bg-neutral-900/50 rounded p-2">
                    {match.method?.service && (
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-purple-400 font-mono">{match.method.service}</span>
                      </div>
                    )}
                    {match.method?.method && (
                      <div className="flex items-center gap-2">
                        <span className="text-neutral-400 ml-4">Method:</span>
                        <span className="text-cyan-400 font-mono">{match.method.method}</span>
                      </div>
                    )}
                  </div>
                ))}
