...
import { streamCombinedLogs, getWorkloadPods, type LogEntry as KubeLogEntry } from '../api/kubernetes/kubernetesLogs';
...
export function useKubernetesLogs({
      ...
      const timer = setTimeout(() => {
        ...
        abortControllerRef.current = streamCombinedLogs({
          ...
          onLog: (log: KubeLogEntry) => {
            // Convert to shared LogEntry format
  ...
  }, [shouldAutoStart, context, namespace, sources, tailLines]);
